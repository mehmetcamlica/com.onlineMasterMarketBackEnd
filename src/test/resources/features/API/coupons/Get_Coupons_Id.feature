Feature: As a provider, I should be able to access the detailed information of the coupon with the specified id number via API connection.
  ...
  Scenario Outline: When a GET request with valid authorization information and correct data (id) is sent to the /api/coupon-details/{id} endpoint, it should be verified that the status code returned is 200 and the response_message in the response body is "Coupon Details".
    ...
    * The api user verifies that the "response.response_message" information in the response body is "Coupon Details", esra.
  ...
  Scenario Outline: When a GET request with valid authorization information and correct data (id) is sent to the /api/coupon-details/{id} endpoint, the data in the response body (id, service_id, coupon_name, coupon_type, coupon_type_text, coupon_percentage, coupon_amount, start_date,end_date, valid_days, user_limit, user_limit_count, description, service_title, status) should be verified.
  ...
  Scenario: When a GET request is sent to the /api/coupon-details/{id} endpoint that does not contain valid authorization information and (id), it should be verified that the status code returned is 203 and the response_message in the response body is "Id missing".
    ...
    * The api user sets "api/coupon-details" path parameters, esra.
    * The api user sends a "GET" request and saves the returned response, esra.
  ...
  Scenario Outline: When a GET request is sent to the /api/coupon-details/{id} endpoint with valid authorization information and an unregistered (id), it should be verified that the status code returned is 203 and the response_message in the response body is "No Details found".

    * The api user sets "api/coupon-details/<id>" path parameters, esra.
    * The api user sends a "GET" request and saves the returned response, esra.
    * The api user verifies that the status code is 203, esra.
    * The api user verifies that the "response.response_message" information in the response body is "No Details found", esra.
  ...
  Scenario Outline: (Invalid Token) When a GET request is sent to the /api/coupon-details/{id} endpoint with invalid (invalid API key) authorization information, it should be verified that the status code returned is 401 and the response_message in the response body is "Invalid token or token missing".
