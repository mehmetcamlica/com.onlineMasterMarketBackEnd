Feature:As a provider, I want to be able to access service and shop categories via API connection.
  ...
  Scenario: When a GET request is sent to /api/categories endpoint with invalid (invalid API key) authorization information, it should beverified that the status code returned is 401 and the response_message in the response body is "Invalid token or token missing".
